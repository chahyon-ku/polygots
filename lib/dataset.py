...
class CoNLLDataset(torch.utils.data.Dataset):
    ...
    def read_data(self, data):
        ...
        for fields, metadata in lib.reader_utils.get_ner_reader(data=data):
            ...
            if len(fields) == 0:
                continue
            sentence_str, tokens_sub_rep, token_masks_rep, coded_ner_, gold_spans_, mask, eos_mask = self.parse_line_for_ner(fields=fields)
            ...
            eos_mask_rep = torch.tensor(eos_mask)

            self.instances.append((tokens_tensor, mask_rep, token_masks_rep, gold_spans_, tag_tensor, eos_mask_rep))
            instance_idx += 1
    ...
    def parse_line_for_ner(self, fields):
        ...
        sentence_str, tokens_sub_rep, ner_tags_rep, token_masks_rep, mask, eos_mask = self.parse_tokens_for_ner(tokens_, ner_tags)
        ...
        return sentence_str, tokens_sub_rep, token_masks_rep, coded_ner_, gold_spans_, mask, eos_mask
    ...
    def parse_tokens_for_ner(self, tokens_, ner_tags):
        ...
        for idx, token in enumerate(tokens_):
            if self._max_length != -1 and len(tokens_sub_rep) > self._max_length:
                break
            if eos_index < 0 and token == self.eos_token:
                eos_index = len(tokens_sub_rep)
        ...
        eos_mask = [True] * eos_index + [False] * (len(tokens_sub_rep) - eos_index) if eos_index >= 0 else [True] * len(tokens_sub_rep)
        return sentence_str, tokens_sub_rep, ner_tags_rep, token_masks_rep, mask, eos_mask

    def collate_batch(self, batch):
        batch_ = list(zip(*batch))
        tokens, masks, token_masks, gold_spans, tags, eos_mask_rep = batch_
        ...
        eos_masks_tensor = torch.zeros(size=(len(tokens), max_len), dtype=torch.bool)
        ...
        for i in range(len(tokens)):
            ...
            eos_masks_tensor[i, :seq_len] = eos_mask_rep[i]

        return token_tensor, tag_tensor, mask_tensor, token_masks_tensor, eos_masks_tensor, gold_spans, 
