...
# LANMAP = {"en": "EN-English", "fr": "FR-French"}
...
def retrieval(name, batch_size=1000):
    data = list()
    with open(f"datasets/{args.lan}/{name}") as file:
        sentence = ""
        entity=""
        for line in file:
            line = line.strip('\n')
    ...
    for i, one in enumerate(data):
        ...
        if len(batch) > 0:
            add_batch(batch, ids)
    with open(f"{VERSION}/{name.split('.')[0]}.txt", mode='w') as writer:
        for one in data:
...
def main():
    for dir_entry in os.scandir(f"datasets/{args.lan}"):
        if dir_entry.name.endswith('.conll'):
            print(dir_entry.name)
            retrieval(dir_entry.name)
    return
