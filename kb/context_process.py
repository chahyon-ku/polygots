...
parser.add_argument('--retrieval_file',
                    help='The retrieved contexts from the knowledge base.')
parser.add_argument(
    '--conll_file', help='The data folder you want to generate contexts, the code will read train, dev, test data in the folder in conll formatting.')
parser.add_argument(
    '--lang', help='The language code of the data, for example "en". We have specical processing for Chinese ("zh") and Code-mixed ("mix").')
parser.add_argument('--use_sentence', action='store_true',
                    help='use matched sentence in the retrieval results as the contexts')
parser.add_argument('--use_paragraph_entity', action='store_true',
                    help='use matched sentence and the wiki anchor in the retrieval results as the contexts')
...
def count_file(file, target='train', is_file=False, max_len=100, comment_label=None):
    ...
    reader.close()
    print(target_file, sent_length.max(), len(
        sentences), (sent_length > max_len).sum())
...
def gen_chinese_search_query(sentences):
    ...
    for sentence in sentences:
        ...
        before_word_is_zh = len(re.findall(
            r'[\u4e00-\u9fff]+', sentence[0].split()[0])) > 0
...
def match_origin_paragraph(sentence, paragraph):
    ...
    try:
        for m in p.finditer(paragraph):
            ...
            # in this case the entity should be in the sentence
            elif end >= span_len + sent_start_pos and end < span_len + sent_start_pos + sentence_length:
...
def process_google(sentences, google_dict, failed_dict, is_conll=False, clean_file=False, full_doc=False, add_eos=False, length_limit=300, for_luke=False, max_rank=999, min_rank=0, lang=None, is_image_retrieval=False, image_ids=None, is_windowed_entity=False, window_size=0, is_wiki_retrieval=False, space_zh=False, is_test=False):
    ...
    for sent_id, sentence in enumerate(sentences):
        ...
        if is_image_retrieval:
            ...
            keywords = gen_ner_based_query2(
                sentence, lang=lang, window=window_size)
            all_contexts = [google_dict[keyword]
                            for keyword in keywords if keyword in google_dict]
            context = []
        ...
        if use_xlmr_tokenization:
            ...
            if lang is not None and lang == 'zh':
                ...
                before_word_is_zh = len(re.findall(
                    r'[\u4e00-\u9fff]+', sentence[0].split()[0])) > 0
                is_eng = len(re.findall(
                    r'[A-Za-z0-9]+', sentence[0].split()[0])) > 0
                add_flag = False
                for word in sentence[1:]:
                    word = word.split()[0]
                    res = re.findall(r'[\u4e00-\u9fff]+', word)
                ...
                temp_keyword2 = ' '.join([word.split()[0]
                                         for word in sentence])
                subtoken_length_count += len(tokenizer.tokenize(temp_keyword2))
            elif is_image_retrieval or is_windowed_entity:
                subtoken_length_count += len(tokenizer.tokenize(
                    ' '.join([word.split()[0] for word in sentence])))
            else:
        ...
        if (lang != 'zh' and lang != 'multi' and not is_windowed_entity and keyword not in google_dict) or (lang == 'zh' and temp_keyword not in google_dict and keyword not in google_dict) or (is_windowed_entity and not success) or (lang == 'multi' and temp_keyword not in google_dict and temp_keyword2 not in google_dict and keyword not in google_dict):
            ...
            if lang is not None:
                ...
                if old_lang is not None:
                    # lang = old_lang
                    failed_dict.append(
                        old_lang+'\t'+old_lang+'_'+str(sent_id)+'\t'+keyword)
                else:
                    failed_dict.append(lang+'\t'+lang+'_' +
                                       str(sent_id)+'\t'+keyword)
                failed_id += 1
            else:
        ...
        if clean_file:
            ...
            if for_luke:
                new_sentence = ['-DOCSTART- O'] + \
                    ['']+sentence+['']+[eos]+end_word
            else:
        ...
        if use_xlmr_tokenization:
            ...
            if len(temp_subtoken) > length_limit:
                pdb.set_trace()
    print(
        f'averaged number of context per sentence: {sum(num_context)/len(num_context)}')
    if sum(num_context) == 0:
        return new_sentences, failed_dict
    print(
        f'number of sentences have contexts: {(np.array(num_context)>=1).sum()}/{len(num_context)}')
    print(
        f'Number of sentences that all context are removed: {removed_context}')
    print(
        f'averaged number of context per DOC sentence: {sum(num_context_for_doc)/len(num_context_for_doc)}')
    print(
        f'distribution number of context per DOC sentence: {sum([x<=20 for x in num_context_for_doc])}, {sum([x>20 for x in num_context_for_doc])}, {max(num_context_for_doc)}')
...
def context_ranking(bow, context_list, origin_sent, removed_context=0) -> list:
    ...
    sorted_context_list = [y[1]
                           for y in sorted(zipped_lists, reverse=True) if y[0] > 0]
    # ==========debug===========
    # zipped_lists = zip(score_list, context_list)
        ...
        for cxt_id, cxt in enumerate(context):
            ...
            for entity in matched_entities:
                indices = [i for i, x in enumerate(
                    potential_spans) if x == entity]
                for index in indices:
                    spans.append(correspond_potential_index[index])
                    correspond_label.append(
                        named_entity_dict[potential_spans[index]])
                # for i in range(len(words)):
                ...
                for spanid, span in enumerate(spans):
                    for pos_id, index in enumerate(span):
                        words[index] = words[index] + ' ' + \
                            correspond_label[spanid][pos_id]
...
for lineid, data in enumerate(google_data):
    ...
    try:
        if use_sentence:
            google_dict, wrong_count, chunked_count = add_to_dict(
                sentence, rank, keyword, google_dict, wrong_count, chunked_count, flag, lang=language, lang_dict=lang_dict)
        else:
            google_dict, wrong_count, chunked_count = add_to_dict(
                desc, rank, keyword, google_dict, wrong_count, chunked_count, flag, lang=language, lang_dict=lang_dict)
        if not is_wiki_retrieval or (is_wiki_retrieval and not use_sentence):
            google_dict, wrong_count, chunked_count = add_to_dict(
                title, rank, keyword, google_dict, wrong_count, chunked_count, flag, lang=language, lang_dict=lang_dict)
    except:
        pdb.set_trace()
...
to_write, train_sentences = count_file(
	args.conll_file, 'train', is_file=True, comment_label='# id')

dataset_name = 'processed_conll_rank_eos'
os.makedirs(dataset_name, exist_ok=True)
is_conll = True
new_train_sentences, failed_dict = process_google(train_sentences, google_dict, failed_dict, is_conll=is_conll, clean_file=False, full_doc=True,
												add_eos='eos' in dataset_name, length_limit=length_limit, for_luke='luke' in dataset_name, lang=args.lang, is_wiki_retrieval=is_wiki_retrieval)
suffix = ''
if args.use_sentence:
	suffix += '_sentence'
if args.use_paragraph_entity:
	suffix += '_withent'
write_file(dataset_name+'/'+os.path.basename(args.conll_file), new_train_sentences, max_len=length_limit)
